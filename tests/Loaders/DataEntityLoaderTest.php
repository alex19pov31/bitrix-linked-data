...
class DataEntityLoaderTest extends TestCase
{
    ...
    public function testGetRepositoryName()
    {
        ...
        $loader = new DataEntityLoader(static::$dataManager, null, $storage);
        $tableName = static::$dataManager->getTableName();
        $repositoryName = $loader::getRepositoryName($tableName);
        $this->assertEquals($repositoryName, 'table_' . $tableName);

        $storage->shouldHaveReceived()->getRepository('table_' . $tableName)->once();
        $storage->shouldHaveReceived()->setLoaderData($loader)->once();

        $loader = new DataEntityLoader(static::$dataManager, 'alternative_name', $storage);
        $storage->shouldHaveReceived()->getRepository('alternative_name')->once();
        $storage->shouldHaveReceived()->setLoaderData($loader)->once();
    }
    ...
    protected function setUp()
    {
        ...
        $dataManager->shouldReceive('getTableName')->andReturn('test_table');
